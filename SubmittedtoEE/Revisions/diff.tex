...
%DIF DEL RecycledPaper_EE.tex      Mon Oct  7 20:44:10 2019
%DIF ADD RecycledPaper_EE-R1.tex   Tue Oct  8 16:38:13 2019
\usepackage{lmodern}
...
\hypersetup{unicode=true,
            pdftitle={Complete tag loss in capture-recapture studies affect abundance estimates: an elephant seal case study},
            pdfborder={0 0 0},
            breaklinks=true}
...
% Uncomment DATE for submission to remove date
  \date{}
%  \predate{}\postdate{}
...
\setlength{\droptitle}{-2em}
  \title{Complete tag loss in capture-recapture studies affects abundance estimates: an elephant seal case study.}
...
known as recycled individuals. For example, an individual with the
 \DIFdelbegin \DIFdel{following tag history over three sampling occasions }\DIFdelend \DIFaddbegin \DIFadd{tag history }\DIFaddend \{11 01 00\} \DIFaddbegin \DIFadd{over three sampling occasions }\DIFaddend was
...
\DIFdelbegin \DIFdel{capture }\DIFdelend \DIFaddbegin \DIFadd{tag }\DIFaddend history \{00 00 11\}. If the rate of tag loss is small, bias in
...
recycled individuals in long-term census data of \DIFdelbegin \DIFdel{elephant seals (}\textit{\DIFdel{Mirounga leonina}}%DIFAUXCMD
\DIFdel{) shown }\DIFdelend \DIFaddbegin \DIFadd{souther elephant seals, similarly }\DIFaddend in Figure 1. 

\includegraphics{RecycledPaper_files/figure-latex/Figure1.jpg}

\begin{quote}
\textsc{Figure 1:}
\textsl{Photograph of elephant seal (\textit{Mirounga leonina}) at Macquarie Island\DIFdelbegin \DIFdel{.}\DIFdelend \DIFaddbegin \DIFadd{, Australia}\DIFaddend }
\end{quote}
...
Full development of the JSTL model is given by \citep{Cowen:2006}.  Data are typically in the form of tag histories for each individual in the study.  Let $\omega_{ijd}$ denote the entries of the tag history for individual $i$ at time $j$, tag $d$; $i=1, \cdots, n_{obs}$, $j=1, \cdots, k$, $d=1,2$; with $\omega_{ijd}$ \DIFdelbegin \DIFdel{equally }\DIFdelend \DIFaddbegin \DIFadd{equaling }\DIFaddend 1 or 0 if the individual was seen with tag $d$ or not.  Capture histories $\omega^*$ used in most capture-recapture studies can be easily obtained from the tag histories $\omega$ and are a series of \DIFdelbegin \DIFdel{1s and 0s }\DIFdelend \DIFaddbegin \DIFadd{1's and 0's }\DIFaddend depicting whether or not an individual was captured at each sample time. We focus on the case with constant parameters as used in our simulation study.  Let $\phi$ denote the survival probability, $p$ the capture probability, $\lambda$ the tag retention probability, and $b_t$ the entry probability for time $t$.  $\Psi$ and $\chi$ are recursive functions of parameters that deal with an individual's history up to the first time see ($f_i$) and after the last time seen ($l_i$) with $n_t$ tags respectively.

The JSTL model is developed under the idea of a \DIFdelbegin \DIFdel{superpopulation }\DIFdelend \DIFaddbegin \DIFadd{super-population }\DIFaddend (the number of individuals that will enter \DIFaddbegin \DIFadd{the }\DIFaddend population at some point during the study) \citep{Schwarz:1996} and this allows the likelihood to be formulated into two parts: 1) a model for the observed number of unique tag histories ($n_{obs}$) given the \DIFdelbegin \DIFdel{superpopulation }\DIFdelend \DIFaddbegin \DIFadd{super-population }\DIFaddend size ($N$) and  2) a model for the recaptures (in the form of capture history frequencies $n_{\omega_i}$) given the observed number of unique tag histories ($n_{obs}$).  The full likelihood is given by the product of these components as follows:
...
\DIFdelbegin \DIFdel{It is not surprising that }\DIFdelend \DIFaddbegin \DIFadd{As expected, the }\DIFaddend survival estimates are \DIFdelbegin \DIFdel{not biased upwards }\DIFdelend \DIFaddbegin \DIFadd{upbiased }\DIFaddend when recycled individuals are \DIFdelbegin \DIFdel{ignored}\DIFdelend \DIFaddbegin \DIFadd{excluded}\DIFaddend .  Survival estimates are essentially a relative measure of how many individuals are around now versus the previous time step.  Thus the bias in the numerator and the denominator essentially cancels out (i.e. $\hat{\phi}_j \approx \hat{N}_{j}/\hat{N}_{j-1}$).	
