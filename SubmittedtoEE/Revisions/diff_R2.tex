...
%DIF DEL RecycledPaper_EE-R1.tex   Thu Oct 10 14:18:40 2019
%DIF ADD RecycledPaper_EE-R2.tex   Fri Dec 20 09:24:43 2019
\usepackage{lmodern}
...
\hypersetup{unicode=true,
            pdftitle={Complete tag loss in capture-recapture studies affects abundance estimates: an elephant seal case study},
            pdfborder={0 0 0},
            breaklinks=true}
...
\setlength{\droptitle}{-2em}
  ...
  %Remove Authors for blind submission
...
\pagestyle{plain}
\pagestyle{fancy}
\chead[]{\chead[]{Complete Tag Loss in Capture-Recapture Studies}}
\cfoot[\thepage]{\thepage}
...
recycled individuals in long-term census data of \DIFdelbegin \DIFdel{souther elephant seals , similarly in Figure 1. 
}\DIFdelend \DIFaddbegin \DIFadd{southern elephant seals (Figure 1). 
}\DIFaddend 
...
\begin{quote}
\textsc{Figure 1:}
\textsl{Elephant seal (\textit{Mirounga leonina}) at Macquarie Island, Australia.}
\end{quote}
...
\noindent where $T_d$ is the probability of being double tagged, $l_{id}$ is the last sample time where tag $d$ was present, and $nt_{j}$ is the number of tags on individual $i$ at time $j$.  A table of notation is provided in \DIFdelbegin \DIFdel{the supplementary material }\DIFdelend \DIFaddbegin \DIFadd{Appendix A }\DIFaddend with further details on the $\Psi$ and $\chi$ functions.
...
this model, we conducted a simulation study. Data sets varied \DIFdelbegin \DIFdel{both }\DIFdelend in super-population size,
...
sizes and double-tagging rates are provided in the \DIFdelbegin \DIFdel{Online Supplement }\DIFdelend \DIFaddbegin \DIFadd{Appendix }\DIFaddend (Figs
...
be found in the \DIFdelbegin \DIFdel{Online Supplement }\DIFdelend \DIFaddbegin \DIFadd{Appendix }\DIFaddend (Figs A7-A17).
...
\begin{quote}
\textsc{Figure 3:}
\textsl{The difference in mean relative bias of the super-population estimate ($\hat{N}$) between the model analyzed including and excluding the recycled individuals for data with super-population size $N=100\ 000$ with 100\% double-tagging for different tag retention probabilities ($\lambda=0.2,0.5,0.9$), survival probabilities (\DIFdelbegin \DIFdel{$\phi=0.2,0.5,0.9$}\DIFdelend \DIFaddbegin \DIFadd{$\phi=0.5,0.9$}\DIFaddend ), and capture probabilities ($p=0.2,0.5,0.9$) using the JSTL model from a ten-sample-time study.}
\end{quote}
...
size (\(N\)) for all scenarios are available in the \DIFdelbegin \DIFdel{Online Supplement }\DIFdelend \DIFaddbegin \DIFadd{Appendix }\DIFaddend (Figs
A19-A24).
...
for all scenarios are available in the \DIFdelbegin \DIFdel{Online Supplement }\DIFdelend \DIFaddbegin \DIFadd{Appendix }\DIFaddend (Figs A25-A42).
...
simulation study. \DIFaddbegin \DIFadd{Standard error estimates for $\hat{N}_t$ are also higher when recycled individuals are included in the analysis. The same pattern is seen in the simulation studies (see  Appendix).
}\DIFaddend 
...
elephant seals is estimated to be $\approx 0.8$ (high).  
\DIFdelbegin \DIFdel{Standard error estimates are also higher when recycled individuals are included in the analysis. The same pattern is seen in the simulation studies (see Web Appendix A).
}\DIFdelend 
