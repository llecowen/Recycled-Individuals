...
Running Head: Complete Tag Loss in Capture-Recapture Studies
...
recycled individuals in long-term census data of southern elephant seals (Figure 1). 
...
\noindent where $T_d$ is the probability of being double tagged, $l_{id}$ is the last sample time where tag $d$ was present, and $nt_{j}$ is the number of tags on individual $i$ at time $j$.  A table of notation is provided in Appendix A with further details on the $\Psi$ and $\chi$ functions.
...
this model, we conducted a simulation study. Data sets varied in super-population size,
...
sizes and double-tagging rates are provided in the Appendix (Figs
...
be found in the Appendix (Figs A7-A17).
...
size (\(N\)) for all scenarios are available in the Appendix (Figs
A19-A24).
...
for all scenarios are available in the Appendix (Figs A25-A42).
...
simulation study. Standard error estimates for $\hat{N}_t$ are also higher when recycled individuals are included in the analysis. The same pattern is seen in the simulation studies (see  Appendix).
...
Alternatively, researchers could replace lost tags on a recaptured individual thereby minimizing the occurrence of complete tag loss.  Depending on model assumptions, the JSTL model may not be appropriate for a study design involving retagging.  Future work would involve extending the JSTL model to  incorporate re-tagged individuals and assess the performance of recycled individuals within this framework. 
  ...
  %Remove Acknowledgments for blind submission
...
Data and R scripts available from the Dryad Digital Repository: \url{https://doi.org/10.5061/dryad.v6wwpzgr3}
...
\newpage
\includegraphics[angle=270,origin=c]{RecycledPaper_files/figure-latex/PDF/Figure1.pdf}
...
Filename: \verb| Figure1.pdf  |

\bigskip


%\includegraphics{RecycledPaper_files/figure-latex/Figure2_survival_GJSTL1-1.jpg}
\includegraphics{RecycledPaper_files/figure-latex/PDF/Figure2_survival_GJSTL1-1.pdf}
\begin{quote}
\textsc{Figure 2:}
...
Filename: \verb| Figure2_survival_GJSTL1-1.pdf  |
\bigskip

\includegraphics{RecycledPaper_files/figure-latex/PDF/Figure3_N-1.pdf}

\begin{quote}
\textsc{Figure 3:}
\textsl{The difference in mean relative bias of the super-population estimate ($\hat{N}$) between the model analyzed including and excluding the recycled individuals for data with super-population size $N=100\ 000$ with 100\% double-tagging for different tag retention probabilities ($\lambda=0.2,0.5,0.9$), survival probabilities ($\phi=0.5,0.9$), and capture probabilities ($p=0.2,0.5,0.9$) using the JSTL model from a ten-sample-time study.}
\end{quote}

Filename: \verb| Figure3_N-1.pdf  |
\bigskip

\includegraphics{RecycledPaper_files/figure-latex/PDF/Figure4_N_k-1.pdf}
...
Filename: \verb| Figure4_N_k-1.pdf  |

\bigskip

\includegraphics{RecycledPaper_files/figure-latex/PDF/Figure5_N_j-1.pdf}
...
Filename: \verb| Figure5_N_j-1.pdf  |
\end{document}
