...
%DIF DEL RecycledPaper_EE.tex      Mon Oct  7 19:40:20 2019
%DIF ADD RecycledPaper_EE-R1.tex   Mon Oct  7 20:20:19 2019
\usepackage{lmodern}
...
%DIF 39c40
%DIF <             pdftitle={The Effect of Recycled Individuals in the Jolly-Seber Model with Tag Loss},
%DIF -------
            pdftitle={Complete tag loss in capture-recapture studies affect abundance estimates: an elephant seal case study}, %DIF > 
%DIF -------
            pdfborder={0 0 0},
            breaklinks=true}
...
\pagestyle{fancy}
%DIF 113c114
%DIF < \chead[]{Recycled individuals in tag-loss models}
%DIF -------
\chead[]{\chead[]{Complete Tag Loss in Capture-Recapture Studies}} %DIF > 
%DIF -------
\cfoot[\thepage]{\thepage}
...
\setlength{\droptitle}{-2em}
  \title{\DIFdelbegin \DIFdel{The effect of recycled individuals in the Jolly-Seber model with tag loss }\DIFdelend \DIFaddbegin \DIFadd{Complete tag loss in capture-recapture studies affect abundance estimates: an elephant seal case study}\DIFaddend .}
  ...
  \author{Emily Malcolm-White\textsuperscript{1},
        ...
        \\ 
        \textsuperscript{1} Mathematics\DIFdelbegin \DIFdel{and Statistics, University of Vermont, Burlington}\DIFdelend \DIFaddbegin \DIFadd{, Middlebury College, Middlebury}\DIFaddend , VT, USA\\ 
...
lost \DIFdelbegin \DIFdel{it's }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend last tag between sample times 2 and 3 and then have been
...
recycled individuals in long-term census data of elephant seals (\textit{Mirounga leonina}) \DIFdelbegin \DIFdel{. For
this particular population, seals could be identified even if they lost
both tags given that they had a permanent brand \mbox{%DIFAUXCMD
\citep{McMahonWhite:2009}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{shown in Figure 1. 
}\DIFaddend 

\DIFaddbegin \includegraphics{Figure1.jpg}

\begin{quote}
\textsc{\DIFadd{Figure 1:}}
\textsl{\DIFadd{Photograph of elephant seal (}\textit{\DIFadd{Mirounga leonina}}\DIFadd{) at Macquarie Island.}}
\end{quote}

\DIFaddend \section{Materials and Methods}\label{methods}

\subsection{The Jolly-Seber Model with Tag Loss
(JSTL)}\label{the-jolly-seber-model-with-tag-loss-jstl}

Full development of the JSTL model is given by \citep{Cowen:2006}\DIFdelbegin \DIFdel{and summarized in the Online Supplement including a table of notation.  
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Many different models can be specified for the JSTL model where
parameters are homogeneous or heterogeneous with respect to time or
group.  We consider the case of the JSTL model where capture, survival, and tag retention probabilities are constant over time\mbox{%DIFAUXCMD
\citep{Cowen:2006}}\hspace{0pt}%DIFAUXCMD
. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Assumptions of the JSTL model with constant survival}\DIFdelend \DIFaddbegin \DIFadd{.  Data are typically in the form of tag histories for each individual in the study.  Let $\omega_{ijd}$ denote the entries of the tag history for individual $i$ at time $j$, tag $d$; $i=1, \cdots, n_{obs}$, $j=1, \cdots, k$}\DIFaddend , \DIFdelbegin \DIFdel{capture, and tag retention probabilities and time-varying entry probabilities are as
follows:
}\DIFdelend \DIFaddbegin \DIFadd{$d=1,2$; with $\omega_{ijd}$ equally 1 or 0 if the individual was seen with tag $d$ or not.  Capture histories $\omega^*$ used in most capture-recapture studies can be easily obtained from the tag histories $\omega$ and are a series of 1s and 0s depicting whether or not an individual was captured at each sample time. We focus on the case with constant parameters as used in our simulation study.  Let $\phi$ denote the survival probability, $p$ the capture probability, $\lambda$ the tag retention probability, and $b_t$ the entry probability for time $t$.  $\Psi$ and $\chi$ are recursive functions of parameters that deal with an individual's history up to the first time see ($f_i$) and after the last time seen ($l_i$) with $n_t$ tags respectively.
}\DIFaddend 
...
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{The effect of recycled individuals is negligible
}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{All individuals (marked and unmarked) are equally catchable, and that
  capture probabilities for all individuals are the same for all
  individuals at all sample times
}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{All individuals (marked and unmarked) have equal survival
  probabilities between all sample times
}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{All individuals have equal entry (birth or immigration) probabilities,
  but entry probabilities can vary between sample times
}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{All marked individuals have equal tag retention probabilities between
  all sample times
}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{For double-tagged individuals, tag loss is independent between tags }%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{There is independence across all individuals
}%DIFDELCMD < \item
\item%DIFAUXCMD
%DIFDELCMD <   %%%
\DIFdel{The sampling period is relatively short compared to the interval
...
%DIFDELCMD < %%%
\subsubsection{\DIFdel{Likelihood and
Estimation}}%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{likelihood-and-estimation}
%DIFDELCMD < %%%
\DIFdelend The JSTL model is developed under the idea of a superpopulation (the number of individuals that will enter population at some point during the study) \citep{Schwarz:1996} and this allows the likelihood to be formulated into \DIFdelbegin \DIFdel{three }\DIFdelend \DIFaddbegin \DIFadd{two }\DIFaddend parts: 1) a model for the observed number of unique tag histories \DIFaddbegin \DIFadd{($n_{obs}$) }\DIFaddend given the superpopulation size (\DIFdelbegin \DIFdel{$L_1^A$) , }\DIFdelend \DIFaddbegin \DIFadd{$N$) and  }\DIFaddend 2) a model for the recaptures \DIFaddbegin \DIFadd{(in the form of capture history frequencies $n_{\omega_i}$) }\DIFaddend given the observed number of unique tag histories (\DIFdelbegin \DIFdel{$L_1^B$), and 3) a model for }\DIFdelend \DIFaddbegin \DIFadd{$n_{obs}$).  The full likelihood is given by the product of these components as follows:
}

\[
\DIFadd{L=\binom{N}{n_\text{obs}} \left\{ \sum_{j=0}^{k-1} b_j(1-p)\chi_{(0,j+1,0)} \right\} ^{(N-n_\text{obs})} \left\{ 1- \sum_{j=0}^{k-1} b_j(1-p)\chi_{(0,j+1,0)} \right\} ^{n_\text{obs}} \times
}\] \[
\DIFadd{\binom{n_\text{obs}}{n_{\omega_1},n_{\omega_2},...,n_{\omega_m}} \prod_{i=1}^{m} \Bigg[ \psi_{f_i} T_d \left\{ \prod_{j=f_i}^{l_i} p^{w*_{ij}} (1-p)^{(1-w*_{ij})} \right\} \left\{ \prod_{j=f_i}^{l_i-1} \phi \right\} \times 
}\] \[
\DIFadd{\prod_{d=1}^{2} \left\{ \left( \prod_{j=f_i}^{l_{id}-1} \lambda \right) \left(1-\prod_{j=l_{id}}^{q_{id}-1} \lambda \right)^{I(l_{id} \neq l_i)} \right\} \times \chi_{f_i, l_i, nt_{l_i}} \Bigg]^{n_{\omega_i}} \times 
}\] \[
\DIFadd{\left\{ 1- \sum_{j=0}^{k-1} b_j(1-p)\chi_{(0,j+1,0)} \right\} ^{-n_\text{obs}}
}\] 

\noindent \DIFadd{where $T_d$ is the probability of being double tagged, $l_{id}$ is the last sample time where tag $d$ was present, and $nt_{j}$ is }\DIFaddend the number of \DIFdelbegin \DIFdel{individuals lost on capture ($L_3$) .  The third component $L_3$,
  is typically used for harvest or fisheries data when known deaths occur.  In this study and in the elephant seal application, we assume there is no possibility of loss on capture, thus the third component of the likelihood simplifies to 1. The
full likelihood is given by the product of the components of the
likelihood (\(L=L_1^A \times L_1^B\)) and can be found in the Online Supplement}\DIFdelend \DIFaddbegin \DIFadd{tags on individual $i$ at time $j$.  A table of notation is provided in the supplementary material with further details on the $\Psi$ and $\chi$ functions.
}

\DIFadd{Assumptions of the JSTL model (under constant $\phi$, $p$, and $\lambda$ parameters) are similar to the Jolly-Seber model \mbox{%DIFAUXCMD
\citep{Schwarz:1996} }\hspace{0pt}%DIFAUXCMD
including all individuals have equal entry (birth or immigration) probabilities,
  but entry probabilities can vary between sample times;  capture probabilities are the same for all individuals at all sample times,  all individuals (marked and unmarked) have equal survival
...
\DIFadd{Many different models can be specified for the JSTL model where parameters are homogeneous or heterogeneous with respect to time \mbox{%DIFAUXCMD
\citep{Cowen:2006} }\hspace{0pt}%DIFAUXCMD
or group  \mbox{%DIFAUXCMD
\citep{Xu:2014}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . 


\DIFaddbegin \subsubsection{\DIFadd{Likelihood and
Estimation}}\label{likelihood-and-estimation}

\DIFaddend Maximum likelihood parameter estimates are found using a Newton-Raphson type method. Estimated standard errors are computed using the delta theorem. Models were implemented using R software \citep{R}\DIFdelbegin \DIFdel{, and code can be obtained from the second author}\DIFdelend \DIFaddbegin \DIFadd{. Code from this study are included in this published article (and its supplementary information files)}\DIFaddend .


\subsection{Experimental Design}\label{experimental-design}

To study the effect of recycled individuals on parameter estimates of
this model, we conducted a simulation study. Data \DIFdelbegin \DIFdel{were simulated and
analyzed using R 3.1.1 \mbox{%DIFAUXCMD
\citep{R}}\hspace{0pt}%DIFAUXCMD
. Data }\DIFdelend sets varied both in super-population size,
...
\DIFdelbegin \DIFdel{doesn't include }\DIFdelend \DIFaddbegin \DIFadd{excludes }\DIFaddend recycled individuals (assumes that individuals, who
...
(once \DIFdelbegin \DIFdel{with and once without }\DIFdelend \DIFaddbegin \DIFadd{including and once excluding }\DIFaddend recycled individuals). We assumed that any
...
The \DIFdelbegin \DIFdel{relative bias of the }\DIFdelend survival estimates are biased for some
...
(Fig. \DIFdelbegin \DIFdel{1}\DIFdelend \DIFaddbegin \DIFadd{2}\DIFaddend ). Box plots of survival estimates for other super-population
...
\DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{including }\DIFaddend and the analysis \DIFdelbegin \DIFdel{without }\DIFdelend \DIFaddbegin \DIFadd{excluding }\DIFaddend the recycled individuals \DIFdelbegin \DIFdel{included }\DIFdelend for both
...
analysis \DIFdelbegin \DIFdel{with and without }\DIFdelend \DIFaddbegin \DIFadd{including and excluding }\DIFaddend recycled individuals for the parameter
...
\DIFdelbegin \DIFdel{Results are similar for both the super-population sizes of 1000 and
100 000 for all parameter combinations of survival, capture, and tag
retention probabilities (Table 1). }\DIFdelend There is slightly more bias due to recycled individuals for parameter combinations 
...
\begin{quote}
\textsc{Table 1:}
\textsl{The mean relative bias of the parameters from the model analyzed \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{including }\DIFaddend ($R$) and \DIFdelbegin \DIFdel{without }\DIFdelend \DIFaddbegin \DIFadd{excluding }\DIFaddend ($R'$) the recycled individuals for data with high survival probability ($\phi=0.9$), high capture probability ($p=0.9$), and low tag retention ($\lambda=0.2$)  for different super-populations sizes ($N=1000,100\ 000$) and different proportion double tagged ($T_2=0.5,1$) using the JSTL model from a ten-sample-time study. }
\end{quote}
...
individuals \DIFaddbegin \DIFadd{(included)}\DIFaddend , the number of observed individuals, \(n_{\text{obs}}\), is larger than it should  be and thus,
\(\hat{N}\) is biased upwards. \DIFdelbegin \DIFdel{This bias is corrected in the analysis
without the recycled individuals considered}\DIFdelend \DIFaddbegin \DIFadd{By recognizing recycled individuals upon recapture, this bias can be corrected}\DIFaddend . The relative bias in the
...
as predicted (Fig. \DIFdelbegin \DIFdel{2}\DIFdelend \DIFaddbegin \DIFadd{3}\DIFaddend , Table 1). The relative bias is small for all
...
individuals is small (less than 0.15) and hence not shown in Fig. \DIFdelbegin \DIFdel{2.
}\DIFdelend \DIFaddbegin \DIFadd{3.
}\DIFaddend SE, SD, and RMSE of \(\hat{N}\) varies, but remains similar between the
...
\begin{quote}
\textsc{Figure \DIFdelbegin \DIFdel{2}\DIFdelend \DIFaddbegin \DIFadd{3}\DIFaddend :}
\textsl{The difference in mean relative bias of the super-population estimate ($\hat{N}$) between the model analyzed \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{including }\DIFaddend and \DIFdelbegin \DIFdel{without }\DIFdelend \DIFaddbegin \DIFadd{excluding }\DIFaddend the recycled individuals for data with super-population size $N=100\ 000$ with 100\% double-tagging for different tag retention probabilities ($\lambda=0.2,0.5,0.9$), survival probabilities ($\phi=0.2,0.5,0.9$), and capture probabilities ($p=0.2,0.5,0.9$) using the JSTL model from a ten-sample-time study.}
\end{quote}

\includegraphics{RecycledPaper_files/figure-latex/Figure3_N_k-1.pdf}

\begin{quote}
\textsc{Figure \DIFdelbegin \DIFdel{3}\DIFdelend \DIFaddbegin \DIFadd{4}\DIFaddend :}
\textsl{Box plots of the estimates of $N$ for the model analyzed \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{including }\DIFaddend and \DIFdelbegin \DIFdel{without }\DIFdelend \DIFaddbegin \DIFadd{excluding }\DIFaddend the recycled individuals for data with super-population size $N=1000$ with 100\% double-tagging for different capture probabilities ($p=0.2,0.5,0.9$), and constant survival ($\phi=0.9$) and tag retention ($\lambda=0.2$) probabilities using the JSTL model from experiments with $k=10, 7,$ and $5$ sample-times. }
\end{quote}

There is more bias in \(\hat{N}\) due to recycled individuals in longer
experiments (Fig. \DIFdelbegin \DIFdel{3}\DIFdelend \DIFaddbegin \DIFadd{4}\DIFaddend ). With a larger number of sampling occasions,
...
\begin{quote}
\textsc{Figure \DIFdelbegin \DIFdel{4}\DIFdelend \DIFaddbegin \DIFadd{5}\DIFaddend :}
\textsl{The difference in mean relative bias of the abundance estimates at each sample time ($\hat{N_j}$) between the model analyzed \DIFdelbegin \DIFdel{with }\DIFdelend \DIFaddbegin \DIFadd{including }\DIFaddend and \DIFdelbegin \DIFdel{without }\DIFdelend \DIFaddbegin \DIFadd{excluding }\DIFaddend the recycled individuals for data with super-population size N=100 000 with 100\% double-tagging for different tag retention probabilities ($\lambda=0.2,0.5,0.9$), survival probabilities (\DIFdelbegin \DIFdel{$\phi=0.2,0.5,0.9$}\DIFdelend \DIFaddbegin \DIFadd{$\phi=0.5$ and $0.9$}\DIFaddend ), and different capture probabilities ($p=0.2,0.5,0.9$) using the JSTL model from a ten-sample-time study.  Note that lines are added between the points to emphasize the difference in values; no models were fit to generate these lines.}
\end{quote}
...
increases (Fig. \DIFdelbegin \DIFdel{4}\DIFdelend \DIFaddbegin \DIFadd{5}\DIFaddend ). For all scenarios, the relative bias in the
...
bias in the survival probability estimates for the analysis \DIFdelbegin \DIFdel{with recycled
individuals  than without }\DIFdelend \DIFaddbegin \DIFadd{including recycled
individuals  }\DIFaddend (Fig. \DIFdelbegin \DIFdel{A14}\DIFdelend \DIFaddbegin \DIFadd{A1-A6}\DIFaddend ) as well as upward bias in the
super-population size estimates. Plots of the mean abundance estimates
for all scenarios are available in the Online Supplement (Figs \DIFdelbegin \DIFdel{A17-A28}\DIFdelend \DIFaddbegin \DIFadd{A19-A42}\DIFaddend ).
...
We considered two \DIFdelbegin \DIFdel{analyzes }\DIFdelend \DIFaddbegin \DIFadd{analyses }\DIFaddend of the data:
...
\item
  ...
  individuals.  \DIFaddbegin \textit{ \DIFadd{Recycled individuals are included.}} \DIFaddend This scenario simulates analysis ignoring the effects of recycled individuals. 
\item
  Recycled individuals were recognized upon recapture (by branding) and were re-tagged with new tags identical to their lost tags. \DIFaddbegin \DIFadd{Thus, }\textit{\DIFadd{recycled individuals are excluded.}} 
...
\DIFdelbegin \DIFdel{with and without }\DIFdelend \DIFaddbegin \DIFadd{including and excluding }\DIFaddend recycled individuals. For comparison to the previous
...
%DIFDELCMD < %%%
\DIFdelend Although the case study of elephant seals validated some of the results from
...
in real life. \DIFdelbegin \DIFdel{Future work could examine }\DIFdelend \DIFaddbegin \DIFadd{For researchers with a particular population in mind, different }\DIFaddend levels of survival, capture, or tag retention \DIFdelbegin \DIFdel{for scenarios of interest for particular populations or
...
individuals}\DIFdelend \DIFaddbegin \DIFadd{could be investigated}\DIFaddend . Additionally, future work could examine the effect of
...
\DIFdelbegin \DIFdel{For researchers interested in conducting and analyzing mark-recapture
...
%DIFDELCMD < %%%
\DIFdelend Developing a model to incorporate recycled individuals is a similar problem to that of incorporating misidentification of individuals.  \cite{Schwarz:1999} developed a model to deal with tag-misreads in an open population capture-recapture setting.  However most of the misidentification literature focusses on genetic or photographic identification errors.  Here multiple identities can be assigned to the same individual leading to overestimates in population size if misidentification is ignored \citep{Yoshizaki:2011}. This is the same result that we see when recycled individuals are ignored.   \cite{Link:2010} introduced the notion of using a latent multinomial to model the latent capture histories for a closed population model. Others have extended \citeauthor{Link:2010}'s model to deal with multiple non-invasive marks \citep{Bonner:2013, McClintock:2013}, heterogeneity in parameters \citep{Mcclintock:2014} and open populations \citep{Bonner:2013}.  These latent multinomial models could be extended to include misidentification produced by complete tag loss.

Finally, the JSTL model we \DIFdelbegin \DIFdel{looked at }\DIFdelend \DIFaddbegin \DIFadd{used }\DIFaddend did not include a component for loss on capture (when for example a fishery harvest occurs).  It would be interesting for future work to include loss on capture to determine if recycled individuals are still problematic under this scenario.  There remains a great deal more to study including testing some of the many assumptions that capture-mark-recapture analyses rely on\DIFdelbegin \DIFdel{, many of which we know are violated in the real world}\DIFdelend .  Increasing computation power and a larger community applying themselves to these problems has the potential to \DIFdelbegin \DIFdel{answer and }\DIFdelend inform researchers and managers in a meaningful way, especially in terms of how we use imperfect observations to estimate vital rates (survival and fecundity). Having more robust estimates of vital rates is especially important if we are to \DIFdelbegin \DIFdel{manage efficiently }\DIFdelend \DIFaddbegin \DIFadd{effectively manage populations on }\DIFaddend an ever increasing list of endangered species\DIFaddbegin \DIFadd{.
}

\DIFadd{For researchers interested in conducting and analyzing mark-recapture
...
researchers are only interested in the survival rates, they do not need
to be concerned with the effect of recycled individuals regardless of
the study's tag-retention rates}\DIFaddend .

\DIFadd{Alternatively, researchers could replace lost tags on a recaptured individual thereby minimizing the occurrence of complete tag loss.  Depending on model assumptions, the JSTL model may not be appropriate for a study design involving retagging.  Future work would involve extending the JSTL model to  incorporate re-tagged individuals and assess the performance of recycled individuals within this framework. }\DIFaddend 
